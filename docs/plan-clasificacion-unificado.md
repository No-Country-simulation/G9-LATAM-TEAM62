...
> **Estado de implementación (actualizado):** ninguno de los 4 niveles está en el código real todavía — no hay `CategoryClassifierService`, `CategoryRule`, tabla de mapeos, ni modelo entrenado. Esto **no bloquea** al Motor de Recomendaciones Presupuestarias (`docs/plan-accion-motor-recomendaciones.md`): ese motor solo lee el campo `category` ya asignado, sin importar qué nivel lo resolvió, así que puede avanzar en paralelo sin esperar esta integración.
