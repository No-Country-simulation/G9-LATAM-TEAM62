...
SENALES_BANCO = {
    "BANCO_CHILE": ["BANCO DE CHILE", "BANCOCHILE"],
    "CUENTA_RUT": ["BANCOESTADO", "BANCO ESTADO", "CUENTARUT", "CUENTA RUT"],
    "FALABELLA": ["BANCO FALABELLA", "FALABELLA"],
...
def detectar_banco(raw: pd.DataFrame, ruta: str | None = None) -> str | None:
    texto = " ".join(normalizar_encabezado(c) for c in raw.head(30).to_numpy().ravel()
                     if not (isinstance(c, float) and np.isnan(c)))
    if ruta:
        texto += " " + normalizar_encabezado(Path(ruta).name)
    puntajes = {banco: sum(s in texto for s in senales) for banco, senales in SENALES_BANCO.items()}
    mejor = max(puntajes, key=puntajes.get)
...
def procesar_cartola(ruta: str, anio_defecto: int | None = None,
    ...
    banco = detectar_banco(raw, ruta)
    reporte["pais"], reporte["anio"], reporte["banco"] = pais, anio_defecto, banco
